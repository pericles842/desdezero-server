...
const PayController = {
  ...
  createUserPayment: async (req, res) => {
    try {
      ...
      if (!("status" in rifa_activa))
        return res
          .status(400)
          .json({ error: "No hay rifas activas, no se puede crear el pago" });
      ...
      } else return res.status(400).json({ error: "No se cargo el pago" });
  ...
  approveSale: async (req, res) => {
    try {
      ...
      let tikes = await TikeModel.generateTickets(
        sales.cantidad_tickets,
        rifa_activa.id
      );

      if (tikes.length == 0)
        return res.status(400).json("Ya no quedan mas tikes disponibles");
      ...
      let tikes_generados_correo =
        sale.tikes != null
          ? sale.tikes.split(",").map((tike) => tike.trim())
          : [];
